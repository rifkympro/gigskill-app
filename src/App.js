...
if(currentUser.role === 'umkm') return ;
...
Buat Akun Baru
Pilih peran Anda untuk bergabung dengan ekosistem.
...
{selectedProject.umkmName} {selectedProject.verified && }

<button onClick={() => setSelectedProject(null)} className="bg-slate-100 p-2 rounded-full text-slate-500 hover:bg-slate-200">
...
Tentang Kami
<button onClick={() => setActiveModal(null)} className="bg-slate-100 p-2 rounded-full text-slate-500 hover:bg-slate-200 hover:text-slate-700 transition-colors">
...
Oleh KELOMPOK 7:
1. RIPKI MAULANA — 251010504293
2. MUHAMAD SOFIYAN — 251010502197
3. FARAH ZAFIRA ROSYADI — 251010502335
4. NAJMA NAURA TSABITA — 251010502277




<button onClick={() => setActiveModal(null)} className="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold py-3.5 rounded-xl transition-all shadow-md">Mengerti & Tutup
