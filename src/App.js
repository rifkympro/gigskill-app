...
const initialProjects = [
  { id: 'p1', umkmId: 'u2', umkmName: 'Toko Kue Ibu', title: "Desain Logo & Banner Toko Kue", budget: "150000", deadline: "3 Hari", desc: "Kami membutuhkan desain logo baru yang minimalis dan banner untuk ditaruh di depan toko fisik kami.", tags: ["Graphic Design", "Illustrator"], verified: true, applicants: [] },
  { id: 'p2', umkmId: 'dummy', umkmName: "Hijab Syar'i Butik", title: "Admin Instagram untuk 1 Minggu", budget: "250000", deadline: "7 Hari", desc: "Tugas meliputi upload feed 1x sehari, membalas DM/Komen (template disediakan), dan buat 3 reels sederhana.", tags: ["Social Media", "Copywriting"], verified: true, applicants: [] },
];

export default function App() {
  const [currentPage, setCurrentPage] = useState('landing'); 
  
  // State In-Memory (Pengganti Database sementara)
  ...
  const handleRegister = (userData) => {
    ...
    showToast(`Pendaftaran ${userData.role === 'student' ? 'Mahasiswa' : 'UMKM'} Berhasil! Silakan login.`, 'success');
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-sans text-slate-900 flex flex-col selection:bg-blue-100 selection:text-blue-900">
      ...
      <main className="flex-grow flex flex-col relative">
        ...
        {currentPage === 'studentDashboard' && currentUser?.role === 'student' && 
          <StudentDashboard currentUser={currentUser} projects={projects} onApply={handleApplyProject} navigateTo={navigateTo} />
        }
        
        {currentPage === 'umkmDashboard' && currentUser?.role === 'umkm' && 
          <UMKMDashboard currentUser={currentUser} projects={projects} onPostProject={handlePostProject} />
        }
        
        {currentPage === 'adminDashboard' && currentUser?.role === 'admin' && 
          ...
          <div className="hidden md:flex items-center space-x-6">
            {!currentUser ? (
              <>
                ...
                <button className="text-sm font-semibold text-slate-500 hover:text-slate-900 transition-colors">Tentang</button>
  ...
  return (
    <div className="animate-in fade-in duration-700">
      <section className="relative pt-20 pb-32 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto overflow-hidden">
        ...
        {/* Dashboard Mockup Image Preview */}
            ...
            <div className="space-y-6">
              ...
              <div className="grid gap-4">
                {projects.map((project) => {
                  ...
                  return (
                    <div key={project.id} onClick={() => !hasApplied && setSelectedProject(project)} className={`bg-white p-6 rounded-2xl border ${hasApplied ? 'border-slate-200 opacity-60 cursor-default' : 'border-slate-200 hover:border-blue-400 hover:shadow-md cursor-pointer'} transition-all group`}>
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          ...
                          <h3 className={`text-xl font-extrabold text-slate-900 ${!hasApplied && 'group-hover:text-blue-600'} transition-colors`}>{project.title}</h3>
                        </div>
...
function UMKMDashboard({ currentUser, projects, onPostProject }) {
  ...
  // Filter project yang dibuat oleh UMKM yang sedang login
...
function Footer() {
  ...
  const modalContent = {
    ...
    panduan: {
      ...
      content: (
        <ul className="list-decimal pl-5 space-y-3 text-slate-600 text-sm">
          <li><strong>Pendaftaran:</strong> Buat akun sebagai Mahasiswa (wajib melampirkan KTM) atau sebagai UMKM.</li>
          <li><strong>Eksplorasi & Posting:</strong> Mahasiswa dapat mencari micro-project yang sesuai dengan keahliannya. UMKM dapat memposting kebutuhan project mereka.</li>
          <li><strong>Proses Lamaran:</strong> Mahasiswa mengirim lamaran ke project yang diminati, dan UMKM memilih kandidat yang paling sesuai.</li>
          <li><strong>Pengerjaan:</strong> Kerjakan project sesuai dengan kesepakatan dan tenggat waktu (deadline).</li>
          <li><strong>Selesai:</strong> Setelah disetujui UMKM, mahasiswa akan mendapatkan bayaran dan ulasan untuk profil mereka.</li>
        </ul>
      )
    },
    syarat: {
      ...
      content: (
        <div className="space-y-3 text-slate-600 text-sm">
          <p>1. <strong>Verifikasi Pengguna:</strong> Mahasiswa wajib melampirkan KTM yang valid dan berstatus aktif. UMKM harus memberikan data usaha yang valid.</p>
          <p>2. <strong>Kewajiban Project:</strong> UMKM wajib memberikan deskripsi tugas dan besaran budget yang jelas sejak awal.</p>
          <p>3. <strong>Transaksi:</strong> Pada fase MVP ini, segala bentuk transaksi keuangan yang dilakukan di luar pantauan platform bukan menjadi tanggung jawab GigSkill.</p>
          <p>4. <strong>Sanksi:</strong> GigSkill berhak penuh memblokir atau menghapus akun pengguna yang terindikasi melakukan penipuan atau melanggar norma yang berlaku.</p>
        </div>
      )
    }
  };
